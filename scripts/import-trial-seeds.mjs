 ...
 *
 * Extracts mock data from all existing trialMockStore/trialData files
 * and inserts into trial_seed.data table via Supabase REST API.
 *
 * Usage: node scripts/import-trial-seeds.mjs
 *
 * Prerequisites:
 * 1. Run supabase/migrations/038_trial_seed_data.sql in Supabase Dashboard SQL Editor
 * 2. This script uses the service_role_key for full access
 */
...
// Override search_path so REST API can find trial_seed schema
const supabase = createClient(
  process.env.SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY,
  {
    db: { schema: 'trial_seed' },
  }
);

// For listing tables (public schema), use a separate client
...
// ─── HELPER: Insert seed data ─────────────────────────────────

async function insertSeed(tableName, records) {
  if (!records || records.length === 0) return { table: tableName, inserted: 0 };

  const now = new Date().toISOString();
  const rows = records.map((r, i) => ({
    table_name: tableName,
    record: { ...r, created_at: now, updated_at: now },
    sort_order: i,
  }));

  const { error } = await supabase.from('data').insert(rows);
  if (error) {
...
async function main() {
  console.log('🚀 Importing trial seed data...\n');

  // Check if trial_seed.data table exists via Supabase REST API
  try {
    const { error } = await supabase.from('data').select('id', { count: 'exact', head: true });
    if (error) {
      console.error('❌ Table trial_seed.data does not exist or is not accessible.');
      console.error(`   Error: ${error.message}`);
      console.error('\n👉 Please run the migration first:');
      console.error('   Go to https://supabase.com/dashboard/project/peslmsctejmvkwzyohke/sql');
      console.error('   Open SQL Editor and run: supabase/migrations/038_trial_seed_data.sql\n');
      process.exit(1);
    }
  } catch (err) {
  ...
  if (count > 0) {
    console.log(`⚠️  Trial seed data already exists (${count} records). Clearing...`);
    const { error } = await supabase.from('data').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    if (error) {
  ...
  console.log('📦 Importing Cashflow seed...');
  for (const [table, records] of Object.entries(cashflowSeed)) {
    await insertSeed(table, records);
  }

  console.log('\n📦 Importing Operations seed...');
  for (const [table, records] of Object.entries(operationsSeed)) {
    await insertSeed(table, records);
  }

  console.log('\n📦 Importing HR seed...');
  for (const [table, records] of Object.entries(hrSeed)) {
    await insertSeed(table, records);
  }

  console.log('\n📦 Importing Inventory seed...');
  for (const [table, records] of Object.entries(inventorySeed)) {
    await insertSeed(table, records);
  }

  // Summary
  const { data: summary } = await supabase.from('data').select('table_name');
  const counts = {};
  for (const row of summary || []) {
    counts[row.table_name] = (counts[row.table_name] || 0) + 1;
  }
  console.log('\n📊 Summary:');
  for (const [table, cnt] of Object.entries(counts).sort()) {
    console.log(`  ${table}: ${cnt} records`);
  }
  const total = Object.values(counts).reduce((a, b) => a + b, 0);
  console.log(`\n✅ Total: ${total} records across ${Object.keys(counts).length} tables`);
}
