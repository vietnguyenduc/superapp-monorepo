-- Adaptive warehouse workspace for zero, one, or many branches.
...
BEGIN
  ...
  IF v_company IS NULL OR v_role<>'admin_company' THEN RAISE EXCEPTION 'Chỉ quản trị công ty được cấu hình kho'; END IF;
  IF p_create_name IS NOT NULL THEN
    ...
    -- With no prior warehouse there is no allocation ambiguity: adopt existing
    -- unassigned history and users into the first warehouse.
