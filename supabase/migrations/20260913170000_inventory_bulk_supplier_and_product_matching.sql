-- Respect company import matching settings and persist the resolved supplier.
...
BEGIN
  ...
  FOR v_row IN SELECT value FROM jsonb_array_elements(p_rows) LOOP
    ...
    IF v_qty<=0 OR COALESCE(v_row->>'date','')='' OR COALESCE(v_row->>'productCode','')='' THEN RAISE EXCEPTION 'Ngày, sản phẩm và số lượng lớn hơn 0 là bắt buộc'; END IF;
    SELECT count(*) INTO v_product_count FROM public.products
      WHERE company_id=p_company_id AND status='active'
        AND CASE WHEN v_match_field='name' THEN lower(trim(name))=lower(trim(v_row->>'productCode')) ELSE business_code=v_row->>'productCode' END;
    IF v_product_count<>1 THEN RAISE EXCEPTION '% sản phẩm đang hoạt động: %',CASE WHEN v_product_count=0 THEN 'Không tìm thấy' ELSE 'Có nhiều hơn một' END,v_row->>'productCode'; END IF;
