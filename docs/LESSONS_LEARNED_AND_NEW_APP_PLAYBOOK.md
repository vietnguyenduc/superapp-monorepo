...
- **`math_factor` trong DB bị ngược hoặc cross-tenant leak**: một công ty khác có `Phát sinh tăng = -1` làm `getMathFactor` trả về sai dấu, gộp `payment` vào phát sinh tăng. Fix: `TransactionTypeContext` phải filter theo active company (dùng `useCompanyId()`); `getMathFactor`/`buildFactorMap` ưu tiên `math_factor` lưu trữ từ type row và chỉ fallback về canonical factor (`charge` +1, `payment`/`refund`/`deposit` -1, `adjustment` +1) khi row thiếu hoặc không có factor; migration sửa `math_factor`/`impact_type` sai và recalc toàn bộ balance, đồng thời `customer_factor_for_type` cast `tt.id` về `text` để tránh lỗi `lower(uuid)`.
