...
This document provides comprehensive documentation for all services, functions, and data structures used in the Cashflow Management System.
...
interface TransactionForm {
  ...
  company_id: string;
  branch_id?: string;
  transaction_type: string;  // transaction_types.id (UUID)
  amount: number;
  description?: string;
...
interface User {
  ...
  position?: string;
  role: UserRole;
  staff_permissions?: Record<string, boolean>; // JSONB
...
interface Transaction {
  id: string;
  transaction_code: string;  // unique per company
  customer_id: string;       // FK → customers.id
  bank_account_id?: string;  // FK → bank_accounts.id
  company_id: string;        // FK → companies.id
  branch_id?: string;       // FK → branches.id
  transaction_type: string; // FK → transaction_types.id (UUID)
  amount: number;
  description?: string;
  reference_number?: string;
  transaction_date: string;
...
// TransactionType is NOT a hardcoded enum — it is dynamically loaded
// per-company from the `transaction_types` table (UUID id + name + color + math_factor).
