@echo off
title ATA Bot — Developer Agent (with Health Check)
cd /d "%~dp0"
set MAX_RESTART=10
set RESTART_COUNT=0
...
if %RESTART_COUNT% GTR %MAX_RESTART% (
    echo [ATA] Max restart limit reached (%MAX_RESTART%). Stopping.
    echo [ATA] %date% %time% - MAX RESTART REACHED >> crash_log.txt
    exit /b 1
)
...
echo [ATA] %date% %time% - Start attempt %RESTART_COUNT% >> crash_log.txt
python main.py

echo [ATA] Bot exited with code %ERRORLEVEL%. Logged.
echo [ATA] %date% %time% - Exit code %ERRORLEVEL% >> crash_log.txt

if %ERRORLEVEL% EQU 0 (
    echo [ATA] Clean exit. Restarting in 3s...
    set RESTART_COUNT=0
    timeout /t 3 /nobreak >nul
) else (
    echo [ATA] Crash detected. Restarting in 10s...
