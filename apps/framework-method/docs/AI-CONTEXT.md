...
- A "Dữ liệu Trí tuệ" (Knowledge Vault) page lives at `/knowledge`. It renders default entries as collapsible accordion groups based on `group_vi`/`group_en` (Sách Vàng table of contents: `A. Mở đầu` → `R. Ví dụ`). Group headers show the group title and entry count; expanding a group lists entries with their summary; clicking an entry expands inline detail (`Cốt ý`, `Cốt ý đầy đủ`, `Cốt của Cốt`, `Lõi`) without a modal. The old `viewing` modal was removed from this page.
...
- `frameworkMethodService.ts` uses `KNOWLEDGE_SEED_VERSION` (currently `8`) and `mergeKnowledgeWithDefaults` so returning users receive the latest cleaned, grouped, expanded seed without clearing `localStorage`.
- The Knowledge page respects `isLoading` from `SessionContext` to avoid flashing the empty state while `knowledgeEntries` are being loaded.
...
- `SessionPage` Step 2 renders the linked knowledge content inside each Concepts/Reference/Examples sub-accordion, with sections and items sorted by their `order_index` from the Builder/template (Nguyên lý → Đạo → Pháp by default); `SECTION_ORDER_BY_BLOCK` and `ITEM_PRIORITY_BY_BLOCK` are only tie-breakers. Items with a linked entry or with inline content show a "Đọc lại dữ liệu" button that opens `KnowledgeModal`.
...
- `KnowledgeEntry` now has `group_vi` and `group_en` fields. The `/knowledge` page groups entries by `group_vi` (Sách Vàng table-of-contents order: `A. Mở đầu` → `R. Ví dụ`) using collapsible group accordions. Group headers show the group title and entry count; clicking a group expands/collapses it, and clicking an entry expands its `Cốt ý`/`Cốt của Cốt`/`Lõi` inline instead of opening a modal.
...
- `knowledgeSeed.ts` was expanded from 43 to 135 entries by extracting the remaining sections of the Sách Vàng OCR (`PHẦN 2 Quy luật` with 6 sub-laws, `PHẦN 3 Hiếu-Lễ-Nghĩa`, `PHẦN 4 Biết gốc & Bám gốc`, `PHẦN 5 Nguyên tắc`, `PHẦN 6 Ý pháp mở rộng & Tâm từ bi`, `PHẦN 7 Xoay chuyển`, `PHẦN 8 Hành`, `PHẦN 9 Luyện & Sửa`, `PHẦN 10 Tu`, `PHẦN 11 Kế thừa`, and the intro/outro concepts). `KNOWLEDGE_SEED_VERSION` was bumped to 7 so returning users receive the expanded library without clearing `localStorage`.
