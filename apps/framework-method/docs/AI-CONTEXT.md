...
- `KnowledgeEntry` exposes `summary_vi/en` (Cốt ý), optional `cot_y_vi/en` (cốt ý sâu), `cot_cua_cot_vi/en` (Cốt của Cốt, shown as list subtitle), and `loi_vi/en` (Lõi / nội dung chi tiết, mirrored to `content_vi`/`content_en` so the session modal and edit form stay in sync). The `/knowledge` form and list show the three main levels and search across them.
