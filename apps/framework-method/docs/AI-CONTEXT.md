...
- `Calendar` shows a `Trổ canh` list with countdowns, status labels, and action buttons; `KarmaActionModal` lets the user `Nhận ra`, `Dừng nghiệp`, or `Giải cảnh` an event, recording a note, image URL, and an editable `Khuôn dừng nghiệp` table.
