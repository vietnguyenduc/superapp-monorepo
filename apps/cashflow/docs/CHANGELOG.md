...
- **Company ID fallback for admin** — `useCompanyId.ts` falls back to `localStorage.selectedCompanyId` and the user's assigned `company_id` for `admin_master`/`admin`, reducing the chance that `Dashboard.tsx` loads cross-tenant data before `CompanyContext` finishes restoring the selection.
