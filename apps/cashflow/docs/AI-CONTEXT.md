...
5. **Customer table responsive UX** — `CustomerList.tsx`/`CustomerTable.tsx` now use a wider `2xl:max-w-screen-2xl` container, reduce mobile padding, and let table columns breathe: explicit `min-w`/`max-w`, wrapped customer names and addresses (`line-clamp-2`), and a visible horizontal scrollbar. Headers wrap on smaller screens. Mobile rows collapse to a card-style view; the table no longer has a fixed mobile height so the page scrolls naturally and avoids the fixed bottom-nav overlap.
