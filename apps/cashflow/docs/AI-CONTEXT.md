...
- **Atomic automatic customer codes:** creation flows with automatic codes call `create_customer_with_auto_code`. The RPC takes a transaction-scoped advisory lock per company, preserves the configured prefix, allocates the code, and inserts the customer before releasing the lock. Trial-mode generation follows the same prefix rule. The code shown while the form is open is only a preview; never use the read-then-insert `generateCustomerCode` path as the final live write boundary.
...
- `src/utils/formatting.ts` — `formatCurrency`, `formatDate`, `getTransactionMathFactor`, `formatBankAccountLabel` (`bank - account_number`), `formatUserLabel` (`Full Name (email)`, falling back to email or `Người dùng #<shortId>`), and `formatShortTransactionCode` (`TXN-<suffix>` display shortener).
...
- **Wide tables / freeze panes** — `CustomerTable.tsx` and `TransactionList.tsx` use `table-fixed w-full` with percentage-based column widths, `overflow-x-visible` wrappers, and a single page scroll (no nested horizontal scroll). The `<thead>` is sticky via `style={{ top: "var(--*-sticky-top, 0px)" }}` while the search/filter bar stays sticky above it; only the header row freezes, not individual body columns.
...
- **Bulk customer import blank-row handling** — `CustomerImport.tsx` filters out rows where all parsed fields (`full_name`, `phone`, `address`, `customer_code`, `working_method`, `notes`, `nguoi_dai_dien`) are empty after parsing. This avoids treating Excel's empty rows inside the used range as invalid rows.
- **Bulk transaction import validation totals** — use `getImportValidationStats` from `src/utils/importUtils.ts` for preview totals. It counts unique invalid row indexes rather than error messages because one row can contain several field errors. `TransactionImport` also validates the exact payload again immediately before calling `bulkImportTransactions`; do not replace that check with preview state alone.
- **Bulk transaction confirmation and receipt** — a valid bulk payload opens a financial-impact confirmation before any write. Successful imports keep a batch receipt on screen and in `cashflow_last_transaction_import_receipt`; validation errors can be downloaded as CSV.
- **Customer detail totals** — `transactionService.getCustomerTransactionSummary` walks the complete paginated customer ledger. Keep this separate from the 50-row recent-history query so labels containing `Tổng` never describe a truncated list.
- **Customer lookup scale** — use `customerService.getAllCustomersForLookup(companyId)` for transaction selectors that must cover every active customer; it paginates in batches of 500.
...
- **Customer list column widths / freeze panes** — in `CustomerTable.tsx`, use `table-fixed w-full` with percentage widths (e.g. `Mã` 7%, `Tên khách hàng` 24%, `Công nợ` 12%, `Giao dịch cuối` 13%, `Thao tác` 10%) so the table fills the viewport without horizontal scroll. Header cells are sticky; body cells use `text-sm` and `px-2 py-2` for readability.
