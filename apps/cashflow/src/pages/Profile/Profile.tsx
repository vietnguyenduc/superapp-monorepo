...
const Profile: React.FC = () => {
  ...
  const isAdmin = user?.role === 'admin' || user?.role === 'admin_master' || user?.role === 'admin_company';
  ...
  const getRoleLabel = (role: string) => {
    ...
    const currentLang = typeof t === 'function' ? 'vi' : t;
    return currentLang === "vi" ? labels.vi : labels.en;
  };
        ...
        <div className="px-6 py-8">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              {!canEditField('full_name') && (
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {t("profile.adminOnlyField", { defaultValue: "Chỉ quản trị viên có thể chỉnh sửa" })}
                </p>
              )}
            </div>
            ...
            {(user?.role === 'admin' || user?.role === 'admin_master' || user?.role === 'admin_company') && (
              <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                  {t("profile.companyInfo", { defaultValue: "Thông tin công ty" })}
                </label>
                <div className="flex items-start gap-4">
                    ...
                    {isEditing && (
                      <label className="cursor-pointer block mt-2">
                        ...
                        <span className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium">
                          {t("profile.uploadLogo", { defaultValue: "Tải lên logo" })}
                        </span>
                      </label>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                      {t("profile.company", { defaultValue: "Tên công ty" })}
                    </label>
                    <input
                      type="text"
                ...
                {user?.company && (
                  <div className="mb-3 overflow-hidden">
                    <span className="text-sm text-gray-500 dark:text-gray-400 break-words">
                      ...
                      <span className="text-gray-900 dark:text-white font-medium break-words">{user.company.name}</span>
                    </span>
                  </div>
                )}
                {user?.branch && (
                  <div className="overflow-hidden">
                    <span className="text-sm text-gray-500 dark:text-gray-400 break-words">
                      ...
                      <span className="text-gray-900 dark:text-white font-medium break-words">{user.branch.name}</span>
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              >
                {isChangingPassword
                  ? t("profile.cancelPassword", { defaultValue: "Hủy đổi mật khẩu" })
                  : t("profile.changePassword", { defaultValue: "Đổi mật khẩu" })}
              </button>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {passwordStatus === "saving"
                        ...
                        : t("profile.updatePassword", { defaultValue: "Cập nhật mật khẩu" })}
                    </button>
                    {passwordStatus === "success" && (
                      <div className="flex items-center text-green-600 dark:text-green-400 text-sm">
                        ...
                        {t("profile.passwordSuccess", { defaultValue: "Đã cập nhật mật khẩu" })}
                      </div>
                    )}
                    {passwordStatus === "error" && (
                      <div className="flex items-center text-red-600 dark:text-red-400 text-sm">
                        ...
                        {t("profile.passwordError", { defaultValue: "Lỗi cập nhật mật khẩu" })}
                      </div>
                    )}
                  </div>
                </div>
