...
// Transaction type normalization, including Vietnamese labels used in imports
export function normalizeTransactionType(type: string): string {
  const normalized = type.toLowerCase().trim();
  if (["payment", "thu", "thanh toán", "thanh toan", "tiền vào", "tien vao", "điều chỉnh giảm", "dieu chinh giam"].includes(normalized)) return "payment";
  if (["charge", "chi", "cho nợ", "cho no", "tiền ra", "tien ra", "điều chỉnh tăng", "dieu chinh tang"].includes(normalized)) return "charge";
  if (["refund", "hoàn tiền", "hoan tien"].includes(normalized)) return "refund";
  if (["adjustment", "điều chỉnh", "dieu chinh"].includes(normalized)) return "adjustment";
  return type;
}

// Parse amount from string/number, handling Vietnamese formats
