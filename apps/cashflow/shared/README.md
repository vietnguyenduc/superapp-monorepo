...
- `auth/`
  - `useTemplateAuth.ts`: Hook pattern with localStorage persistence and typed state.
  - `AuthProviderTemplate.tsx`: Context wrapper exposing auth state/actions.
  - `ProtectedRouteTemplate.tsx`: Guard for protected routes.
- `i18n/`
...
- Namespace localStorage keys per app (e.g., `appname_darkMode`, `appname_language`).
