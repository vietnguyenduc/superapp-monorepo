...
APP_DATA_ENTRY_GUIDE = {
    ...
    "hr-operation": {
        "types": ["👤Thông tin nhân viên", "📅Chấm công", "🏖 Nghỉ phép"],
        "format": "Excel/CSV hoặc paste từ Google Sheets",
...
def send_welcome(message):
    ...
    welcome_text = (
        ...
        f"6️⃣ `/settings` - Cấu hình hệ thống, AI model, autopilot.\n"
    )
...
# --- Inline Button Action Handlers (from main) ---
...
def handle_select_app_confirm(call):
    """Saves selected app to USER_APP_STATE and shows confirmation."""
    bot.answer_callback_query(call.id)
    user_id = call.from_user.id
...
def handle_schedule_action(call):
    ...
    bot.send_message(
        ...
        "Dùng `/settings` để cấu hình autopilot và schedule.",
        reply_markup=markup,
        parse_mode="Markdown"
    )
...
def handle_document(message):
    ...
    try:
        ...
        if filename.lower().endswith(('.csv', '.xlsx', '.xls')):
            ...
            bot.reply_to(
                message,
                f"✅ **Tệp dữ liệu `{filename}` đã được tải lên thành công!**\n\n"
                f"📂 **Đường dẫn nội bộ**: `{path}`\n\n"
                f"💡 **Gợi ý xử lý**:\n"
                f"Anh/Chị có thể yêu cầu: *'Hãy phân tích file này'* hoặc *'Map dữ liệu file này vào database'*. Tôi sẽ tự động dùng công cụ Data Profiler để khám sức khỏe dữ liệu trước khi import.",
                parse_mode="Markdown"
            )
        else:
            bot.reply_to(
