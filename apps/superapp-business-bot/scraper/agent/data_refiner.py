...
class DataRefiner:
    def __init__(self, use_local_ollama: bool = True):
        """
        Khởi tạo DataRefiner.
        - Tự động kiểm tra nếu Local Ollama đang chạy, nếu không sẽ tự động fallback sang DeepSeek/Gemini Cloud API.
        """
        load_unified_env()
        ...
        if use_local_ollama and ollama_running:
            print("[+] Đang khởi tạo AI kết nối với Local Ollama (qwen2.5-coder)")
            self.client = OpenAI(
            ...
            if deepseek_key:
                ...
                print("[+] Local Ollama ngoại tuyến. Tự động chuyển vùng sang Cloud API (Gemini-2.5-Flash)...")
                self.client = OpenAI(
                ...
                print("[+] Đang khởi tạo AI kết nối với Cloud API (OpenAI/Gemini)")
                self.client = OpenAI(
