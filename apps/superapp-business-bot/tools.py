...
def export_report_csv(args: dict) -> str:
    """
    Exports a cleaned or raw dataframe to a CSV file in the vault for the user to download.
    Args:
        args: {
            "source_type": "google_sheet" | "csv" | "excel",
            "path_or_url": "str",
            "rules": list (optional, transformations to apply),
            "output_filename": "str" (e.g. "error_report.csv")
        }
    """
    import asyncio
    import os
    ...
    output_filename = args.get("output_filename", "export.csv")
    
    if not source_type or not path_or_url:
        return "Error: missing source_type or path_or_url"
    ...
    try:
        ...
        # Resolve vault path
        try:
            with open(Path(__file__).parent / "active_project.json", "r") as f:
                import json
                active_project = json.load(f).get("active_project", "default")
        except Exception:
        ...
        if source_type == "google_sheet":
            ...
            with open(path_or_url, "rb") as file_obj:
                content = file_obj.read()
            df = loop.run_until_complete(parse_file(content, os.path.basename(path_or_url)))
        else:
            return f"Error: unsupported source_type '{source_type}'"
        ...
        return f"File exported successfully to {output_path.absolute()}\nTell the user to download it using this syntax exactly: [FILE: {output_filename}]({output_path.absolute()})"
