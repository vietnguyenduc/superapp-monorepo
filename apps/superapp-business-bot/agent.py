...
LOCAL_TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Profiles raw data from Google Sheets, CSV, or Excel files using Pandas. Use this to determine data types and identify dirty data (nulls, mixed types) BEFORE deciding on a mapping strategy.",
            "parameters": {
                "type": "object",
                "properties": {
                    "args": {
                        "type": "object",
                        "description": "Arguments for profiling",
                        "properties": {
                            "source_type": {
                                "type": "string",
                                "enum": ["google_sheet", "csv", "excel"],
                                "description": "The type of data source."
                            },
                            "path_or_url": {
                                "type": "string",
                                "description": "The file path (if csv/excel) or URL (if google_sheet)."
                            }
                        },
...
LOCAL_TOOLS_SCHEMA.extend([
    {
        "type": "function",
        "function": {
            "name": "clean_and_preview_data",
            "description": "Applies a set of JSON transformation rules to the data and returns a new profile + 5 rows preview. Do this BEFORE inserting into database to let user verify.",
            "parameters": {
                "type": "object",
                "properties": {
                    "source_type": {"type": "string", "enum": ["google_sheet", "csv", "excel"]},
                    "path_or_url": {"type": "string"},
                    "rules": {
                        "type": "array",
                        "items": {"type": "object"},
                        "description": "Array of transformation rules. Supported actions: rename_columns, fillna, replace, cast, drop_columns, drop_nulls."
                    }
                },
                "required": ["source_type", "path_or_url", "rules"],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "insert_to_supabase",
            "description": "Applies the approved transformation rules and bulk inserts the cleaned data into Supabase.",
            "parameters": {
                "type": "object",
                "properties": {
                    "source_type": {"type": "string", "enum": ["google_sheet", "csv", "excel"]},
                ...
                "required": ["source_type", "path_or_url", "rules", "table_name"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": "Exports a cleaned or raw dataframe to a CSV file in the vault for the user to download.",
            "parameters": {
                "type": "object",
                "properties": {
                    "source_type": {"type": "string", "enum": ["google_sheet", "csv", "excel"]},
                    "path_or_url": {"type": "string"},
                    "rules": {"type": "array", "items": {"type": "object"}},
                    "output_filename": {"type": "string", "description": "e.g. error_report.csv"}
                },
                "required": ["source_type", "path_or_url", "output_filename"],
            },
        },
    }
])
...
class AntigravityAgent:
    def __init__(self):
        ...
        self.system_instruction = (
            "You are the AI Business Operations Manager (Siêu Trợ Lý Vận Hành). Your primary role is to assist users (Sales, HR, Accounting, Managers) with business tasks, data entry, reports, and workflow automation. You are NOT a software developer.\n"
            ...
            "Guidelines:\n"
            "1. You act as an elite AI Assistant. Always be polite, professional, and helpful. Refer to yourself as 'Trợ lý' and to the user as 'Anh/Chị'.\n"
            "2. NEVER apologize or claim that you cannot inspect files, databases, or systems. You are equipped with 100% full, unrestricted local terminal and file access through your local tools (execute_command, read_file, write_file, list_directory).\n"
            "3. If a task requires database operations (like Supabase data retrieval, querying metrics, checking inventory), use your tools to query data directly via `execute_command` (e.g., using `npx supabase db query` or executing python scripts).\n"
            "4. Execute tool calls immediately when needed—be proactive! If the user asks you to generate a report, retrieve a record, or check data, immediately execute the appropriate tool instead of asking for permission or explaining how they can do it themselves.\n"
            "5. Keep Telegram responses concise, clear, and business-focused. Use formatting (bolding, lists) to make data easy to read.\n"
            "6. CRITICAL ON FORMATTING: When presenting tabular data (like reports, logs, lists), ALWAYS format it as a raw ASCII table wrapped inside a standard Markdown code block (using triple backticks with 'text' language: ```text \n ... \n ```). This preserves monospaced alignment on Telegram mobile and makes it beautiful to read. NEVER use standard Markdown tables (like `|---|`) outside of a code block because Telegram renders them poorly.\n"
            "7. CRITICAL ON PORTS & SERVERS: Whenever you need to start a dev server (e.g., Vite, Next.js, Express), YOU MUST do 2 things: (a) Explicitly kill any existing zombie processes on that port FIRST. (b) NEVER start the server synchronously (which blocks your execution loop forever). You MUST run it as a background task. If using execute_command, use powershell `Start-Process npx -ArgumentList \"vite --port 5180 --host\" -NoNewWindow` or use the tool's built-in async features (e.g., WaitMsBeforeAsync). DO NOT pipe output to `| head` to 'preview' it because the server won't exit when the pipe closes on Windows and will hang your terminal.\n"
            "8. You can use semantic_search or read_file to read company documents, manuals, and internal procedures to answer questions.\n"
            "9. CRITICAL: When starting servers (like `npx vite`), you MUST run it as a background task. Do NOT use `start /B` or `&` or synchronous commands because they hang the python process (stdout pipe remains open). Do NOT use `npx` directly with Start-Process because it throws 'not a valid Win32 application'. Instead, you MUST use this exact format: `Start-Process cmd -ArgumentList \"/c npx vite --port 5180 --host\" -WindowStyle Hidden`. This is the ONLY way to start a server on Windows without blocking the agent.\n"
            "10. CRITICAL — PATH CONVENTION FOR list_directory / read_file / write_file:\n"
            ...
            "15. DATA INGESTION PROTOCOL & TELEGRAM UX:\n"
            "    - Khi người dùng gửi 1 file (Excel/CSV) hoặc 1 link Google Sheet, HÃY GỌI NGAY công cụ `profile_raw_data` để khám sức khỏe dữ liệu trước.\n"
            "    - Đọc Data Profile: Nếu phát hiện data bị bẩn, hãy báo cáo lại. ĐỂ HIỂN THỊ ĐẸP TRÊN TELEGRAM, hãy luôn format các sample/bảng tóm tắt dưới dạng bảng Markdown chuẩn (ví dụ: `| Cột 1 | Cột 2 |`). Telegram bot sẽ tự động biến bảng Markdown thành bảng ASCII monospaced.\n"
            "    - Đối với các lỗi phức tạp hoặc file quá lớn, hãy chủ động gọi tool `export_report_csv` để xuất báo cáo. Sau đó gửi đường dẫn cho người dùng theo CÚ PHÁP CHÍNH XÁC NÀY: `[FILE: Tên file hiển thị](đường_dẫn_tuyệt_đối_trả_về_từ_tool)`. Telegram bot sẽ tự động bắt lấy thẻ này và đính kèm file document gửi cho user.\n"
            "    - Khi người dùng yêu cầu 'tự động fix/sửa', hãy tạo bộ luật JSON và GỌI tool `clean_and_preview_data`. Trình bày preview cho user bằng bảng Markdown.\n"
            "    - Sau khi user duyệt, HÃY GỌI `insert_to_supabase` để chèn vào Database.\n"
            "16. PLANNING & EXECUTION PROTOCOL (CRITICAL FOR COMPLEX TASKS):\n"
