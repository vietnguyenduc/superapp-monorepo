...
export function mockSupabaseChain(config: {
  ...
  // The select chain: select() returns orderChainObj (since most queries do select().order() then await)
  const selectChain = orderChainObj;
  ...
  // Insert chain: insert() returns { select: () => chain that resolves to singleThenable }
  ...
  insertSelectChainObj.order = vi.fn().mockReturnValue(orderChainObj);
  ...
  insertSelectChainObj.neq = vi.fn().mockReturnValue(neqChainObj);
  insertSelectChainObj.in = vi.fn().mockReturnValue(inChainObj);

  const insertMock = vi.fn().mockReturnValue({
    select: vi.fn().mockReturnValue(insertSelectChainObj),
    then: selectThenable.then,
    catch: selectThenable.catch,
    finally: selectThenable.finally,
  });

  // Update chain: update() returns { eq: () => { select: () => chain(singleThenable) } }
  const updateMock = vi.fn().mockReturnValue({
    eq: vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue(singleChainObj),
  ...
  return {
    ...
    // Chain-level mocks for multi-step chains
    gteChainInMock: gteChainObj.in,
    lteChainInMock: lteChainObj.in,
    inChainNeqMock: inChainObj.neq,
    neqChainOrderMock: neqChainObj.order,
  };
}
