...
export class InventoryMovementService extends BaseService {
  static async createMovement(movement: Omit<InventoryMovement, 'id' | 'created_at' | 'updated_at'>): Promise<ServiceResponse<InventoryMovement>> {
    return this.execute(
      async () => {
        ...
        return await apiClient.from('inventory_movements').insert([{ ...movement, company_id: movement.company_id || companyId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }]).select().single();
      },
  ...
  static async getMovements(filters: any = {}): Promise<ServiceResponse<InventoryMovement[]>> {
    return this.execute(
      async () => {
        const companyId = filters.companyId || await getCurrentCompanyId();
  ...
  static async getMovementsByReference(referenceId: string, sourceType: string): Promise<ServiceResponse<InventoryMovement[]>> {
    return this.execute(
      async () => {
        const companyId = await getCurrentCompanyId();
        let query = apiClient.from('inventory_movements').select('*').eq('source_id', referenceId).eq('source_type', sourceType);
        if (companyId) query = query.eq('company_id', companyId);
        return await query.order('movement_date', { ascending: false });
      },
  ...
  static async createBalanceSnapshot(snapshot: Omit<InventoryBalanceSnapshot, 'id' | 'created_at' | 'updated_at'>): Promise<ServiceResponse<InventoryBalanceSnapshot>> {
    return this.execute(
      async () => {
        const companyId = await getCurrentCompanyId();
        return await apiClient.from('inventory_balance_snapshots').insert([{ ...snapshot, company_id: snapshot.company_id || companyId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }]).select().single();
      },
  ...
  static async getCurrentBalance(companyId: string, productId: string): Promise<ServiceResponse<{ quantity: number; value?: number }>> {
    return this.execute(
      async () => {
        ...
        if (!currentCompanyId) return { data: { quantity: 0 } };
        const { data, error } = await apiClient.from('inventory_movements').select('running_balance, running_value').eq('company_id', currentCompanyId).eq('product_id', productId).order('movement_date', { ascending: false }).order('movement_time', { ascending: false }).limit(1).single();
