...
const SettingsPage: React.FC = () => {
  ...
  const [salesMode, setSalesMode] = useState<SalesMode>(settings.salesMode || 'b2b');
  ...
  const handleToggleSalesMode = (mode: SalesMode) => {
    setSalesMode(mode);
    appSettingsService.saveSettings({ salesMode: mode });
  };
  ...
  const tabs = [
    ...
    { key: 'sales-mode', label: 'Chế độ Bán hàng (POS / B2B)', description: 'Chuyển đổi giao diện bán hàng (POS siêu tốc vs B2B chi tiết).' },
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {/* Tab: Sales Mode */}
            {activeTab === 'sales-mode' && (
              <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} rounded-2xl border shadow-sm p-6`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    { id: 'b2b', label: 'B2B (Bán buôn / Đơn hàng lớn)', icon: '📦', desc: 'Giao diện chi tiết dạng lưới (Grid), nhập hàng loạt, quản lý công nợ khách hàng.' },
                    { id: 'pos', label: 'POS (Bán lẻ / Tại quầy)', icon: '🛍️', desc: 'Giao diện tính tiền nhanh, quét mã vạch, in bill trực tiếp.' }
                  ].map(model => (
                    <button
                      key={model.id}
                      onClick={() => handleToggleSalesMode(model.id as SalesMode)}
                      className={`p-6 rounded-2xl border-2 text-left transition-all relative ${
                        salesMode === model.id
                          ? 'border-blue-500 bg-blue-500/10 shadow-lg'
                          : (isDarkMode ? 'border-gray-700 bg-gray-700/30 hover:border-gray-600' : 'border-gray-100 bg-white hover:border-gray-200')
