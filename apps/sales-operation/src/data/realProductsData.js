...
export const convertToProductInterface = (realProduct) => {
    ...
    const isFinished = realProduct.type === "Thành phẩm";
    const nameLower = realProduct.name.toLowerCase();
    // Logic permissions & forms
    let allowedForms = [];
    let canBePurchased = true;
    let canBeSold = true;
    let rawToProcessedRatio = 1;
    let processedToFinishedRatio = 1;
    if (isFinished) {
        allowedForms = ['finished'];
        canBePurchased = true;
        canBeSold = true;
    }
    else {
        // Bán thành phẩm / NVL
        if (nameLower.includes('dĩa') || nameLower.includes('đĩa') || nameLower.includes('combo')) {
            allowedForms = ['finished'];
            canBePurchased = false; // Cannot purchase a platter, must produce it
            canBeSold = true;
            processedToFinishedRatio = typeof realProduct.inputRatio === 'number' ? realProduct.inputRatio : 1;
        }
        else if (nameLower.includes('miếng') || nameLower.includes('gram') || nameLower.includes('phần')) {
            allowedForms = ['raw', 'processed'];
            canBePurchased = true; // Can purchase raw materials
            canBeSold = false; // Usually don't sell raw components directly
            rawToProcessedRatio = typeof realProduct.inputRatio === 'number' ? realProduct.inputRatio : 1;
        }
        else {
            allowedForms = ['raw'];
            canBePurchased = true;
    ...
    return {
        ...
        canBeSold: canBeSold,
        rawToProcessedRatio: rawToProcessedRatio,
        processedToFinishedRatio: processedToFinishedRatio,
        recipe: [], // To be populated if needed
