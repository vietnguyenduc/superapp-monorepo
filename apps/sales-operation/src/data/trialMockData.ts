 ...
 */
export const RAW_TO_FINISHED_MAP: Record<string, string[]> = {
...
export const RAW_CONVERSIONS_MAP: Record<string, any[]> = {
  'NVL-XO01': [
...
export const INTERMEDIATE_UNITS_MAP: Record<string, string[]> = {
  'NVL-XO01': ['miếng'],
  'NVL-DH01': ['miếng'],
  'NVL-DU01': ['miếng'],
  'DK-SU01': ['muỗng'],
...
export const RECIPE_MAP: Record<string, any[]> = {
  'TP-XO01': [
...
export function getTrialProducts(): Product[] {
  ...
  if (!raw) { 
    ...
    if (!rawAfterSeed) return [];
    return JSON.parse(rawAfterSeed).map((p: any) => ({
      ...p,
...
export function saveTrialProducts(products: Product[]): void {
  localStorage.setItem(LS_KEY_PRODUCTS, JSON.stringify(products));
}

export function saveTrialInventoryRecords(records: InventoryRecord[]): void {
...
export function getFinishedProductsForCode(businessCode: string, allProducts: Product[]): Product[] {
