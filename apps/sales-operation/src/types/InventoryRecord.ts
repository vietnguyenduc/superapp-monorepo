...
export interface InventoryRecord {
  ...
  outputQuantity?: number; // Xuất sổ (Kế toán kho)
  ...
  // Thông tin kế toán (Nhập kho)
  supplierId?: string; // ID Nhà cung cấp (Partner ID)
  supplierName?: string; // Tên nhà cung cấp (Lưu cache)
  unitPrice?: number; // Đơn giá nhập
  totalAmount?: number; // Thành tiền
  invoiceImage?: string; // Ảnh hóa đơn/chứng từ
  approvalStatus?: 'pending' | 'approved' | 'rejected'; // Trạng thái phê duyệt giá (Price Variance Approval)
  priceVariancePercentage?: number; // % Chênh lệch so với giá chuẩn
...
export interface InventoryReport {
  ...
  // Tổng tồn thực tế (Cộng dồn đơn thuần)
  totalActualStock: number;
  
  // Tồn quy đổi (Tính theo định lượng)
