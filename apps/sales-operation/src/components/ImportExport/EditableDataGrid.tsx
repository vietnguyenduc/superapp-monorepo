...
const EditableDataGrid: React.FC<EditableDataGridProps> = ({
  ...
  const handlePaste = useCallback(
    (e: React.ClipboardEvent<HTMLTableElement>) => {
      ...
      // If we have existing data, replace it; otherwise add new data
      if (data.length === 0 || (data.length === 1 && Object.values(data[0]).every(v => !v))) {
        onDataChange(pastedData);
      } else {
        onDataChange([...data, ...pastedData]);
      }
    },
  ...
  // Render cell content
  const renderCell = (rowIndex: number, columnKey: string, value: string, column: any) => {
    ...
    return (
      <div
        className={`px-3 py-2 cursor-pointer hover:bg-gray-50 min-h-[36px] flex items-center ${
        ...
        <span className={`${hasError ? 'text-red-700' : 'text-gray-900'}`}>
          {value || (
            <span className="text-gray-400 italic">
              {column.type === 'select' ? 'Chọn giá trị...' : 'Nhập dữ liệu...'}
            </span>
          )}
        </span>
        {hasError && (
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div className="text-sm text-gray-600">
          {data.length} dòng dữ liệu • Click vào ô để chỉnh sửa • Paste (Ctrl+V) để dán dữ liệu
        </div>
