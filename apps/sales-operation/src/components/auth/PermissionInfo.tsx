...
export const PermissionInfo: React.FC = () => {
  ...
  const permissionGroups = {
    'Nhập liệu tồn kho': permissions.filter(p => p.includes('inventory_input')),
    'Danh mục hàng hóa': permissions.filter(p => p.includes('product_catalog')),
    'Báo cáo bán hàng': permissions.filter(p => p.includes('sales_report')),
    'Xuất đặc biệt': permissions.filter(p => p.includes('special_outbound')),
    'Báo cáo nhập xuất': permissions.filter(p => p.includes('inventory_report')),
    'In phiếu kiểm kho': permissions.filter(p => p.includes('stock_check')),
    'Dashboard': permissions.filter(p => p.includes('dashboard')),
    'Cài đặt': permissions.filter(p => p.includes('settings') || p.includes('user_management')),
    'Hệ thống': permissions.filter(p => p.includes('system') || p.includes('audit')),
  };
