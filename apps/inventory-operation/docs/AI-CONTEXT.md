...
- **Migration history reconciled:** production contains the RLS policies, branch columns, transfer table and Inventory RPCs introduced by migrations `20260912230438` through `20260915120000`. Their remote history entries were repaired on 2026-09-19 after schema inspection and an isolated InsForge PostgreSQL compile/workflow smoke test. Do not rerun those migration files. Two-tenant authorization verification is still required before a real-data pilot.
...
- **Procurement safety:** `/procurement` separates PO, GR and supplier returns. Creating a PO never writes stock. A supplier return is pending until an `admin_company`/`admin_master` approves it; only completion writes canonical outbound ledger rows. Do not bypass the `inventory_*procurement*` RPC boundaries.
- **Procurement migration:** `20260919173822_inventory_procurement_workflows.sql` was applied on production on 2026-09-20. It adds return line items, branch-scoped procurement policies, decimal PO quantities and the procurement RPC boundaries.
...
- **Adaptive warehouse workspace:** `WarehouseWorkspaceProvider` handles zero/one/many active branches. Zero shows first-warehouse setup to `admin_company` and `admin_master`; creating the first branch adopts unassigned users and history. One is selected silently; many show the working-warehouse picker and transfer navigation. Staff never switch their assigned branch.
...
- **Demand rate:** MRP uses `calculateDailyOutput`; preserve fractional daily demand and exclude future-dated records. Do not round demand before calculating DOH or suggested purchase quantity. Selected MRP rows must navigate directly to `/procurement?tab=po` with their suggested quantities and supplier prefilled.
- **Outbound types:** `import_export_config.outboundTypes` is company-scoped and drives the standard single/bulk goods-issue reason selector and its template. Do not create a parallel special-outbound workflow in navigation.
