  ...
  const handleMultiple = async (rows: any[]) => {
    ...
    for (const row of rows) {
      ...
      const result = await createRecord({
        ...
        supplierName: globalSupplier || row.supplierName,
        invoiceImage: globalInvoice ? globalInvoice.name : undefined,
      });
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
          {/* Global Supplier/Invoice Inputs for Accountant when doing Bulk/Multiple */}
          {viewRole === 'warehouse_accountant' && activeTab !== 'single' && (
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6 transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-black text-blue-700 uppercase tracking-widest mb-2">
                    🏭 Nhà Cung Cấp Chung (Cashflow App)
                  </label>
                  <select
                    value={globalSupplier}
                    onChange={(e) => setGlobalSupplier(e.target.value)}
                    className="w-full px-4 py-2.5 bg-white border border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                  >
                    <option value="">Chọn NCC từ Cashflow App...</option>
                    {suppliers.map(s => (
                      <option key={s.customer_code} value={s.customer_code}>{s.full_name}</option>
                    ))}
                  </select>
                  <p className="text-[10px] text-blue-500 mt-1">Áp dụng cho toàn bộ danh sách khi import</p>
                </div>
                <div>
                  <label className="block text-xs font-black text-blue-700 uppercase tracking-widest mb-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 bg-white border border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200 cursor-pointer"
                  />
                </div>
              </div>
              <p className="text-xs text-blue-600 mt-3 font-medium">
                * Nếu điền Nhà cung cấp chung và Hóa đơn, hệ thống sẽ ưu tiên áp dụng cho toàn bộ danh sách khi Import.
              </p>
            </div>
          )}
