...
interface GoodsReceiptBulkGridProps {
  onSave: (rows: BulkGoodsReceiptRow[]) => void;
  onCancel: () => void;
  isLoading?: boolean;
}
...
const COLS = [
  ...
  { key: 'unit_price', label: 'Đơn giá', width: 100 },
  { key: 'notes', label: 'Ghi chú', width: 160 },
] as const;

const emptyRow = (_i: number): BulkGoodsReceiptRow => ({
...
const GoodsReceiptBulkGrid: React.FC<GoodsReceiptBulkGridProps> = ({
  onSave,
  onCancel,
  isLoading = false,
}) => {
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    const newRows: BulkGoodsReceiptRow[] = lines.map((line, i) => {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-sm text-blue-800 dark:text-blue-300">
        ...
        <ul className="list-disc pl-8 space-y-1 mb-3">
          ...
          <li>
            Hoặc tải template, điền dữ liệu, rồi upload file
          </li>
        </ul>
...
export default GoodsReceiptBulkGrid;
