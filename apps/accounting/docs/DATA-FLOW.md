...
1. User opens `accounting.appforyou.xyz` and logs in via `@superapp/iam`.
2. `CompanyProvider` resolves `company_id` / `branch_id`.
3. Components call local service modules:
   - `api` builds Supabase queries scoped by tenant.
   - `backupHistoryService` builds Supabase queries scoped by tenant.
   - `bankAccountService` builds Supabase queries scoped by tenant.
   - `branchService` builds Supabase queries scoped by tenant.
   - `businessLogic/calculations` builds Supabase queries scoped by tenant.
   - `businessLogic/index` builds Supabase queries scoped by tenant.
   - `businessLogic/transformation` builds Supabase queries scoped by tenant.
   - `businessLogic/validation` builds Supabase queries scoped by tenant.
