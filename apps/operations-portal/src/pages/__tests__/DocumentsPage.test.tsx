import { describe, it, expect, vi, beforeEach } from 'vitest';
import { render, screen } from '@testing-library/react';
import { MemoryRouter } from 'react-router-dom';

// Mock supabase
vi.mock('../../lib/supabase', () => ({
  supabase: {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        order: vi.fn(() => Promise.resolve({ data: null, error: null })),
      })),
      insert: vi.fn(() => Promise.resolve({ error: null })),
    })),
    storage: {
      from: vi.fn(() => ({
        upload: vi.fn(() => Promise.resolve({ error: null })),
        createSignedUrl: vi.fn(() => Promise.resolve({ data: { signedUrl: 'https://test.url' }, error: null })),
      })),
    },
    auth: {
      getUser: vi.fn().mockResolvedValue({ data: { user: { id: 'test-user' } }, error: null }),
    },
  },
  TABLES: {
    OPERATION_DOCUMENTS: 'operation_documents',
    USERS: 'users',
  },
  STORAGE: { OPERATIONS_MEDIA: 'operations_media' },
  getCurrentUser: vi.fn().mockResolvedValue({ id: 'test-user' }),
  getCurrentUserId: vi.fn().mockResolvedValue('test-user'),
}));

// Mock trialData
vi.mock('../../lib/trialData', () => ({
  isTrialMode: vi.fn(() => false),
  mockDocuments: [],
}));

describe('DocumentsPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders the page title', async () => {
    const DocumentsPage = (await import('../DocumentsPage')).default;
    render(
      <MemoryRouter>
        <DocumentsPage />
      </MemoryRouter>
    );
    expect(screen.getByText('Tài liệu & Quy định')).toBeInTheDocument();
  });

  it('renders the upload button', async () => {
    const DocumentsPage = (await import('../DocumentsPage')).default;
    render(
      <MemoryRouter>
        <DocumentsPage />
      </MemoryRouter>
    );
    expect(screen.getByText('+ Đăng tài liệu mới')).toBeInTheDocument();
  });

  it('shows loading state initially', async () => {
    const DocumentsPage = (await import('../DocumentsPage')).default;
    render(
      <MemoryRouter>
        <DocumentsPage />
      </MemoryRouter>
    );
    expect(screen.getByText('Đang tải tài liệu...')).toBeInTheDocument();
  });

  it('shows empty state when no documents', async () => {
    const DocumentsPage = (await import('../DocumentsPage')).default;
    render(
      <MemoryRouter>
        <DocumentsPage />
      </MemoryRouter>
    );
    const emptyText = await screen.findByText('Chưa có tài liệu nào.');
    expect(emptyText).toBeInTheDocument();
  });
});
